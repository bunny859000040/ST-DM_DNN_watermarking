...
from tensorflow.python.keras.utils.layer_utils import convert_all_kernels_in_model, convert_dense_weights_data_format
...
def DenseNet(input_shape=None, depth=40, nb_dense_block=3, growth_rate=12, nb_filter=-1, nb_layers_per_block=-1,
             ...
             classes=10, activation='softmax', watermark_regularizer=None):
    ...
    x = __create_dense_net(classes, img_input, include_top, depth, nb_dense_block,
                           ...
                           dropout_rate, weight_decay, subsample_initial_block, activation, watermark_regularizer)
    ...
    if weights == 'imagenet':
        ...
        if weights_loaded:
            ...
            if tf.keras.backend.image_data_format() == 'channels_first' and tf.keras.backend.backend() == 'tensorflow':
...
def DenseNetImageNet121(input_shape=None,
    ...
    return DenseNet(input_shape, depth=121, nb_dense_block=4, growth_rate=32, nb_filter=64,
                    ...
                    classes=classes, activation=activation, watermark_regularizer=WM_regularizer)
...
def __conv_block(ip, nb_filter, bottleneck=False, dropout_rate=None, weight_decay=1e-4, watermark_regularizer=None, name=None):
...
def __dense_block(x, nb_layers, nb_filter, growth_rate, bottleneck=False, dropout_rate=None, weight_decay=1e-4,
                  grow_nb_filters=True, return_concat_list=False, watermark_regularizer=None, name=None, target_id=1):
    ...
    for i in range(nb_layers):
        if i == 0 and target_id == 1 and watermark_regularizer:
            cb = __conv_block(x, growth_rate, bottleneck, dropout_rate, weight_decay, watermark_regularizer=watermark_regularizer, name=name)
        else:
...
def __create_dense_net(nb_classes, img_input, include_top, depth=40, nb_dense_block=3, growth_rate=12, nb_filter=-1,
                       ...
                       subsample_initial_block=False, activation='softmax', watermark_regularizer=None):
    ...
    for block_idx in range(nb_dense_block - 1):
        if block_idx == 1 and watermark_regularizer:
          x, nb_filter = __dense_block(x, nb_layers[block_idx], nb_filter, growth_rate, bottleneck=bottleneck,
                                       dropout_rate=dropout_rate, weight_decay=weight_decay,
                                       watermark_regularizer=watermark_regularizer, name='WM_conv', target_id=block_idx)
        else :
          x, nb_filter = __dense_block(x, nb_layers[block_idx], nb_filter, growth_rate, bottleneck=bottleneck,
                                       dropout_rate=dropout_rate, weight_decay=weight_decay, target_id=block_idx)
    ...
    x, nb_filter = __dense_block(x, final_nb_layer, nb_filter, growth_rate, bottleneck=bottleneck,
                                 dropout_rate=dropout_rate, weight_decay=weight_decay, target_id=3)
...
if __name__ == '__main__':
    ...
    scale = 0.01
    b = np.ones((1, 256))
    WM = custom_WM_regularizer(b=b, scale=scale)
    model = DenseNet((32, 32, 3), depth=100, nb_dense_block=3,
                     growth_rate=12, bottleneck=False, reduction=0.5, weights=None, watermark_regularizer=WM)
    model.summary()
