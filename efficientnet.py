...
def mb_conv_block(inputs, block_args, activation, drop_rate=None, prefix='', watermark_regularizer=None):
    ...
    if watermark_regularizer:
        x = layers.DepthwiseConv2D(block_args.kernel_size,
                                   ...
                                   depthwise_regularizer=watermark_regularizer,
                                   name=prefix + 'dwconv_WM')(x)
    else :
        x = layers.DepthwiseConv2D(block_args.kernel_size,
                                   strides=block_args.strides,
                                   padding='same',
...
def EfficientNet(width_coefficient,
    ...
    for idx, block_args in enumerate(blocks_args):
        ...
        if idx == 2 and watermark_regularizer != None:
            x = mb_conv_block(x, block_args,
                              activation=activation,
                              drop_rate=drop_rate,
                              prefix='block{}a_'.format(idx + 1),
                              watermark_regularizer=watermark_regularizer)
        else:
            x = mb_conv_block(x, block_args,
                              activation=activation,
                              drop_rate=drop_rate,
...
def EfficientNetB1_224x224(include_top=True,
    ...
    return EfficientNet(1.0, 1.1, 224, 0.2,
                        ...
                        include_top=include_top, weights=weights,
                        input_tensor=input_tensor, input_shape=input_shape,
                        pooling=pooling, classes=classes, watermark_regularizer=watermark_regularizer,
                        **kwargs)
...
def EfficientNetB4_224x224(include_top=True,
                           ...
                           classes=1000,
                           watermark_regularizer=None,
                           watermark_regularizer2=None,
                           **kwargs):
    return EfficientNet(1.4, 1.8, 224, 0.4,
                        model_name='efficientnet-b4',
                        include_top=include_top, weights=weights,
                        input_tensor=input_tensor, input_shape=input_shape,
                        pooling=pooling, classes=classes, watermark_regularizer=watermark_regularizer, watermark_regularizer2=watermark_regularizer2,
                        **kwargs)
...
if __name__ == '__main__':
    ...
    image_dim = (224, 224, 3)
    WM = l2(1e-4)
    model = EfficientNetB1_224x224(input_shape=image_dim, watermark_regularizer=WM)
    model.summary()